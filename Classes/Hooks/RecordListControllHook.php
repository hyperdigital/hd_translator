...
class RecordListControllHook
{
     ...
     */
    public function renderListEntry($table, $rowUid)
    {
        if ($rowUid instanceof \TYPO3\CMS\Recordlist\RecordList\DatabaseRecordList ) {
            return $this->renderListEntryIcon($table);
        } else {
            return '';
        }
    }
    ...
    /**
     * view of the record list icon
     *
     * @param $table - table[0] = tablename, table[1] = uid of row
     * @return string
     */
    protected function renderListEntryIcon($table)
    {
